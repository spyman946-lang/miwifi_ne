...
class MiWifiEntity(CoordinatorEntity):
    ...
    def __init__(
        ...
        _tk: str | None = getattr(description, "translation_key", None)
        _slug_part: str = (
            (_tk or description.key) if description.name is None else description.name
        )
        ...
        if _tk is not None:
            self._attr_translation_key = _tk
        if getattr(description, "has_entity_name", False):
